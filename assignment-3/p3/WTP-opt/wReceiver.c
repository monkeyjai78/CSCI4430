      ...
      if ((recv_len = recvfrom(sock, buffer, PACKET_LENGTH, MSG_NOSIGNAL, (struct sockaddr *) &si_sender, &slen)) == -1) {
        ...
        if (in_connection && recv_header.type == PACKET_TYPE_DATA) {
          ...
          if (recv_header.seqNum  >= head_seqNum && recv_header.seqNum < head_seqNum + window_size && isPacketValid) {
            /* If the received seqNum is within the window (i.e. not out of window) and
          ...
          if (recv_header.seqNum < head_seqNum + window_size && isPacketValid) {
            ...
            // Find the next head seqNum
            int next_head_seqNum = -1;
            for (int i = 0; i < window_size; i++) {
              if (window[i].length == 0) {
                // window[i].length == 0 implies the packet refering to (i + 1)th slot of the window is empty and thus not received
                // Assign the next head seqNum to be that of the first unreceived packet in the current window
                next_head_seqNum = head_seqNum + i;
                break;
              }
                next_head_seqNum = head_seqNum + window_size;
            }
            
            /* If the next head seqNum is not within the window,
               assign next head seqNum to be head_seqNum + window_size */
            if (next_head_seqNum == -1) next_head_seqNum = head_seqNum + window_size;

            printf("next_head_seqNum: %d\n", next_head_seqNum);
            printf("head_seqNum: %d\n", head_seqNum);
            
            if (next_head_seqNum > head_seqNum) {
              /* If the next head seqNum is behind the current head seqNum
                 (i.e. packets with seqNum < next head seqNum are all received),
                 move the window forward such that the next head seqNum become the new head seqNum */

              // Write the data of those packets which are going to slide out from the window
              write_window(fp, window, window_size, 0, next_head_seqNum - head_seqNum - 1);
              // Move the window forward
              set_window(
                window,
                window_size,
                next_head_seqNum - head_seqNum < window_size ? next_head_seqNum - head_seqNum : -1, // set -1 to renew the whole window
                window_size - 1
              );
              // Update the head seqNum
              head_seqNum = next_head_seqNum;
            }

            // Send back the ACK if the received checksum of the received packet is valid
            send_ack(recv_header.seqNum, sock, (struct sockaddr *) &si_sender, slen, log);
            printf("Sending ACK to %s:%d, seqNum: %d\n", inet_ntoa(si_sender.sin_addr), ntohs(si_sender.sin_port), recv_header.seqNum);
          }
        }
      }
    }
