...
void send_file(char * input_file, int window_size, int sock, struct sockaddr_in * dest_attr, socklen_t dest_len, char * log) {
  ...
  while (1) {
      ...
      if (recv_header.type == PACKET_TYPE_ACK) {
        ...
        if (recv_header.seqNum > head_seqNum && recv_header.seqNum <= head_seqNum + window_size) {
          ...
          set_window(
            ...
            recv_header.seqNum - head_seqNum < window_size ? recv_header.seqNum - head_seqNum : -1, // set -1 to renew the whole window
            window_size - 1
          );
          ...
          send_window(window, window_size, head_seqNum + window_size - recv_header.seqNum, window_size - 1,
            recv_header.seqNum, sock, dest_attr, dest_len, log);
          // Update the head seqNum
          head_seqNum = recv_header.seqNum;
    ...
    // Check retransmission timer for resending packets
    gettimeofday(&rt_end, NULL);
    double passed_time = (rt_end.tv_sec - rt_start.tv_sec) * 1000 + (rt_end.tv_usec - rt_start.tv_usec) / (double) 1000;
    // Retransmission timeout
    if (passed_time > RETRANSMISSION_TIME) {
      // If the counted time exceeds the retransmission time, resend the whole window's packets
